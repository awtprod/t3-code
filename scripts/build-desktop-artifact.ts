...
/**
 * Length of the UNC share-root prefix (double backslash, host, backslash, share),
 * or 0 when the path is not UNC.
 *
 * The share is the highest real directory on a UNC path: the double-backslash host
 * prefix on its own is not one, so the ancestor walk must stop there.
 */
...
export function ancestorNodeModulesPaths(
  ...
  // (double-backslash host, backslash, share) and drive roots keep their prefix instead of
  // being rebuilt into a relative path that silently resolves against the build cwd.
  const paths: string[] = [];
  let current = trimTrailingSeparators(startDir);
  // On a UNC path the share itself is the root: the double-backslash host prefix is not a
  // directory, so walking past the host+share prefix would emit paths that cannot exist.
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    // hand-packed server.asar sidecar (see WINDOWS_SERVER_ASAR_RESOURCE) when
    // the build includes a local/WSL backend.
    extraResources: [
      ...DESKTOP_EXTRA_RESOURCES,
      ...(platform === "win" && !remoteOnlyBuild ? WINDOWS_SERVER_EXTRA_RESOURCES : []),
      ...(platform === "win" && !remoteOnlyBuild && wslRuntimeBundled
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  // the server.asar sidecar (see stageWindowsServerSidecar). A remote-only
  // build ships no server on any platform, so it needs only the desktop
  // runtime deps too. macOS (non-remote-only) adds only server packages that
  // remain external to its merged app.asar. Linux (non-remote-only) retains
  // its existing full dependency tree.
  const stageDependencies =
    options.platform === "win" || remoteOnlyBuild
  ...
  // sidecar (which embeds the Linux node-pty prebuild); other platforms, and
  // remote-only builds (which stage no server at all), ignore the prebuild
  // input. windowsServerAsarPath is already undefined for a remote-only build.
  ...
  // packaged backends do, rather than by reasoning about the emitted source.
  //
  ...
  //
  // Only Windows ships anything outside app.asar; macOS and Linux keep the
  // whole tree inside the asar, where this check has nothing to look at.
  // Windows validates and executes the separately packed server sidecar after
  // electron-builder copies it into the final payload. A remote-only build has
