...
Local desktop development uses `commandcenter-dev://app`, while packaged builds use `commandcenter://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `commandcenter-dev://app`; the production Clerk instance should only need
`commandcenter://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["commandcenter://app"]}'
```
...
The production macOS bundle ID is `com.awtprod.commandcenter`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.awtprod.commandcenter` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.awtprod.commandcenter`.
...
T3CODE_PORT=13773 \
  "/Applications/Command Center.app/Contents/MacOS/Command Center"
```
...
```sh
codesign --verify --deep --strict "/Applications/Command Center.app"
codesign -d --entitlements :- "/Applications/Command Center.app"
```
