 ...
 * `org/project/repo`, and a URL can arrive in any form: `https://example.com/owner/
 * repo.git`, `ssh://git@example.com:22/owner/repo`, `git@example.com:owner/repo.git`, with
...
export function getCloneDirectoryName(repositoryOrRemoteUrl: string | null | undefined): string {
  ...
  const hasHost =
    schemeIndex >= 0 ||
    new RegExp(`^[^/${BACKSLASH}${BACKSLASH}:]+@[^/${BACKSLASH}${BACKSLASH}:]+:`).test(
      withoutQuery,
    );
  const pathPart = schemeIndex >= 0 ? withoutQuery.slice(schemeIndex + "://".length) : withoutQuery;
  const segments = pathPart
    .split(new RegExp(`[/${BACKSLASH}${BACKSLASH}:]+`))
  ...
  // repository in `https://example.com/acme/123` really is named `123`.
