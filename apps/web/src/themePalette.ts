...
// library cannot drift on what a published theme may be called. Vercel Dark is
// a Command Center built-in that upstream's shared set does not know about, so
// its id is reserved on top of that source rather than instead of it.
...
/**
 * The status colors Command Center shows without a theme, read from the app's own
...
// Vercel Dark is a Command Center built-in that the shared library does not
// carry, so splice it in beside the flagship palette rather than appending it
// after the managed themes.
const BUILT_IN_THEME_DEFINITIONS: ReadonlyArray<ThemeDefinition> = [
  T3_CHAT_THEME,
  VERCEL_DARK_THEME,
  ...BUILT_IN_THEMES.filter((definition) => definition.id !== T3_CHAT_THEME_ID),
];
