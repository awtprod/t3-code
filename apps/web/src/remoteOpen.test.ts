...
const TAILSCALE_TARGETS = [
  { kind: "tailscale", host: "sol.example-tailnet.ts.net" },
...
describe("resolveRemoteOpenState", () => {
  ...
  it("uses deep links for a primary target reached over the network", () => {
    expect(
      resolveRemoteOpenState({
        target: primaryTarget("https://sol.example-tailnet.ts.net"),
        sshAlias: null,
        isDesktopRenderer: false,
        remoteOpenTargets: TAILSCALE_TARGETS,
      }),
      ...
      mode: "remote-links",
      host: { kind: "tailscale", host: "sol.example-tailnet.ts.net" },
    });
  });
  ...
  it("keeps exec behavior for the desktop app's own primary even on a NAT URL", () => {
    ...
    expect(
      resolveRemoteOpenState({
        target: primaryTarget("http://203.0.113.1:14369"),
        sshAlias: null,
...
describe("buildRemoteOpenUrl", () => {
  it("builds a vscode-remote deep link", () => {
    expect(
      buildRemoteOpenUrl({
        editor: "vscode",
        host: "sol.example-tailnet.ts.net",
        absolutePath: "/workspace/example-project/code/my repo",
      }),
    ).toBe(
      "vscode://vscode-remote/ssh-remote+sol.example-tailnet.ts.net/workspace/example-project/code/my%20repo",
    );
  });
