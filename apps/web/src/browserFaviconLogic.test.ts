...
describe("browser favicon logic", () => {
  it("keys valid origins canonically while keeping distinct scopes separate", () => {
    ...
    expect(faviconKey("env:project", "http://192.0.2.2:3000/", "192.0.2.2")).toBe(
      faviconKey("env:project", "http://localhost:3000/", "192.0.2.2"),
    );
    ...
    const keys = [
      ...
      faviconKey("env:a", "http://192.0.2.50:3000/", "192.0.2.2"),
    ];
    ...
    expect(faviconKey("env:a", ["http:/", "local:3000/"].join("/"), null)).not.toBe(
      faviconKey("env:a", "http://localhost:3000/", null),
    );
  });
  ...
  it("retains an exact environment-host alias for offline lookup", () => {
    ...
    expect(faviconStorageLocation("env:project", "http://localhost:3000/app", "192.0.2.2")).toEqual(
      expected,
    );
    expect(faviconStorageLocation("env:project", "http://192.0.2.2:3000/app", "192.0.2.2")).toEqual(
      expected,
    );
    expect(faviconStorageLocation("env:project", "https://example.com/app", "192.0.2.2")).toEqual({
      aliases: [],
      key: "env:project https://example.com:443",
    });
  ...
  it("evicts old entries and sanitizes hydrated state", () => {
    ...
    expect(
      migratePersistedBrowserFaviconState({
        byKey: {
          [["env:project http:/", "local:3000"].join("/")]: entry(4),
      ...
      byKey: {
        ...
        "env:project http://localhost:3003": {
          ...entry(6),
          aliases: ["192.0.2.2", "192.0.2.3", "192.0.2.4", "192.0.2.5"],
        },
      },
    });
  });
