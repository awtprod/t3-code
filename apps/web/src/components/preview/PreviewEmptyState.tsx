  ...
  if (servers.length === 0 && recents.length === 0) {
    return (
      <Empty>
        ...
        <EmptyDescription>
          Type a URL above, or run a web app in this environment. Its listening ports will show up
          here automatically.
        </EmptyDescription>
      </Empty>
    );
        ...
        {servers.length > 0 ? (
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <h2 className="font-medium">Environment servers</h2>
            </div>
            <div className="flex flex-col divide-y divide-border/60 overflow-hidden rounded-xl border border-border/70 bg-background">
              {servers.map((server) => (
            ...
            <p className="px-1 text-xs text-muted-foreground">
              Select a listening port to open it from this environment in this browser tab.
            </p>
          </div>
        ) : null}
      </div>
