...
describe("MessagesTimeline", () => {
  ...
  it("shows the animated one-line label for a live tool group", () => {
    ...
    // The fork's working row flips to "may be stuck" after a silence, so a live
    // turn needs a live clock; the fixed fixture date would read as stalled.
    const liveStartedAt = new Date().toISOString();
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        isWorking
        activeTurnStartedAt={liveStartedAt}
        lastActivityAt={liveStartedAt}
        latestTurn={{
          turnId,
          state: "running",
