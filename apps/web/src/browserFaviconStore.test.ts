...
describe("browser favicon store", () => {
  ...
  it("finds a persisted environment icon after shell hydration without a live connection", () => {
    recordFaviconForProject(projectRef, favicon("http://192.0.2.2:3000/app", 5), "192.0.2.2");
    ...
    expect(lookupFavicon(byKey, projectRef, "http://192.0.2.2:3000/app", null)).toBe(PNG);
    expect(lookupFavicon(byKey, projectRef, "http://192.0.2.3:3000/app", null)).toBeNull();
    expect(lookupFavicon(byKey, projectRef, "https://192.0.2.2:3000/app", null)).toBeNull();
    expect(lookupFavicon(byKey, projectRef, "http://192.0.2.2:3001/app", null)).toBeNull();
    expect(
      lookupFavicon(
        byKey,
        scopeProjectRef(environmentId, ProjectId.make("project-2")),
        "http://192.0.2.2:3000/app",
        null,
      ),
    ).toBeNull();
  });
  ...
  it("finds a persisted IPv6 environment icon without a live connection", () => {
    recordFaviconForProject(projectRef, favicon("http://[2001:db8::1]:3000/app", 5), "2001:db8::1");
    const migrated = migratePersistedBrowserFaviconState({
      byKey: useBrowserFaviconStore.getState().byKey,
    }).byKey;
    expect(lookupFavicon(migrated, projectRef, "http://[2001:db8::1]:3000/app", null)).toBe(PNG);
  });
  ...
  it("keeps exact host aliases attached to the newest canonical icon", () => {
    ...
    recordFaviconForProject(
      projectRef,
      favicon("http://192.0.2.2:3000/", 10, olderPng),
      "192.0.2.2",
    );
    recordFaviconForProject(
      projectRef,
      favicon("http://192.0.2.3:3000/", 20, newerPng),
      "192.0.2.3",
    );
    recordFaviconForProject(
      projectRef,
      favicon("http://192.0.2.4:3000/", 15, olderPng),
      "192.0.2.4",
    );
    const byKey = useBrowserFaviconStore.getState().byKey;
    for (const host of ["192.0.2.2", "192.0.2.3", "192.0.2.4"]) {
  ...
  it("evicts an icon and its exact host aliases atomically", () => {
    recordFaviconForProject(projectRef, favicon("http://192.0.2.2:3000/", 100), "192.0.2.2");
  ...
  it("retains multiple origins until project and connection metadata hydrate", () => {
    ...
    expect(flushPendingFaviconsForThread(threadRef, projectRef, "192.0.2.2")).toBe(true);
  ...
  it("persists unambiguous loopback captures while the environment is offline", () => {
    ...
    recordFaviconForThread(threadRef, favicon("http://192.0.2.2:5173/", 2), projectRef, undefined);
