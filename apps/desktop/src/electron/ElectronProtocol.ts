...
async function staticClientResponse(
  ...
  try {
    ...
    return new Response(body.buffer, {
      headers: {
        "content-type": staticClientContentTypes[path.extname(filePath).toLowerCase()] ??
          ...
          try: () => {
            Electron.protocol.handle(input.scheme, (request) =>
              input.staticClientRoot
                ? staticClientResponse(request, input.staticClientRoot, fileSystem, path).then((response) =>
