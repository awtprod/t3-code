...
const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  ...
  // either home var must not have this one command silently target the default
  // install (see cli/config.ts explicitBaseDir resolution).
  const commandCenterHome = Option.filter(
    yield* envCommandCenterHome,
    (value) => value.trim().length > 0,
  );
  const t3Home = Option.filter(yield* envT3Home, (value) => value.trim().length > 0);
  const envHome = Option.orElse(commandCenterHome, () => t3Home);
