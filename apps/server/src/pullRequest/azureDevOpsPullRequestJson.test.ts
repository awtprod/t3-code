...
function pullRequest(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    createdBy: { displayName: "Bilal Hassan", uniqueName: "bilal@example.com" },
...
describe("decodePullRequestListJson", () => {
  it("reads a pull request as a change request", () => {
    ...
    expect(batch.items[0]).toMatchObject({
      ...
      author: { login: "bilal@example.com", name: "Bilal Hassan" },
  ...
  it("reads reviewers as review requests", () => {
    const detail = expectSuccess(
      decodePullRequestJson(
        asJson(
          pullRequest({
            reviewers: [{ displayName: "Julius", uniqueName: "julius@example.com", vote: 10 }],
          }),
        ),
      ),
    );

    expect(detail?.reviewRequestLogins).toEqual(["julius@example.com"]);
    expect(detail?.reviewers).toEqual([
      { login: "julius@example.com", name: "Julius", avatarUrl: null },
    ]);
  });
...
describe("decodeViewerJson", () => {
  it("reads the signed-in account name", () => {
    expect(expectSuccess(decodeViewerJson(asJson({ user: { name: "bilal@example.com" } })))).toBe(
      "bilal@example.com",
    );
  });
...
describe("decodeThreadsJson", () => {
  it("takes every real comment of every thread, oldest first", () => {
              ...
              comments: [
                {
                  ...
                  author: { displayName: "Julius", uniqueName: "julius@example.com" },
            ...
            },
            {
              ...
              comments: [
                ...
                { id: 1, content: "Bilal voted", commentType: "system", publishedDate: "x" },
                {
                  ...
                  author: { displayName: "Bilal", uniqueName: "bilal@example.com" },
    ...
    expect(comments[0]).toMatchObject({
      kind: "issue-comment",
      author: { login: "bilal@example.com" },
    });
  });
