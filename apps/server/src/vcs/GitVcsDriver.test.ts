...
it.effect("GitVcsDriver trusts only the exact workspace path for each command", () => {
  ...
  return Effect.gen(function* () {
    ...
    yield* driver.execute({
      operation: "GitVcsDriver.test.safeDirectory",
      cwd,
      args: ["status"],
    });

    assert.deepStrictEqual(observedArgs, ["-c", `safe.directory=${cwd}`, "-C", cwd, "status"]);
    assert.isFalse(observedArgs?.includes("safe.directory=*"));
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        NodeServices.layer,
