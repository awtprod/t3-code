    ...
    Effect.gen(function* () {
      ...
      const countingSpawner = ChildProcessSpawner.make((command) =>
        Effect.gen(function* () {
          ...
          const gitDirArgumentIndex = command.args.indexOf("--git-dir");
          const isRemoteNamesScan =
            gitDirArgumentIndex >= 0 &&
            command.args[gitDirArgumentIndex + 2] === "remote" &&
            gitDirArgumentIndex + 3 === command.args.length;
      ...
      yield* driver.initRepo({ cwd });
      yield* runGit(["config", "user.email", "test@example.com"]);
    ...
    Effect.gen(function* () {
      ...
      const partiallyFailingSpawner = ChildProcessSpawner.make((command) =>
        Effect.gen(function* () {
          ...
          const branchArgumentIndex = command.args.indexOf("branch");
          if (
            branchArgumentIndex >= 0 &&
            command.args[branchArgumentIndex + 1] === "feature/partial-failure"
          ) {
            const handle = yield* delegate.spawn(command);
  ...
  describe("worktree operations", () => {
    ...
    it.effect.skip("checks out submodules in a new worktree", () =>
      Effect.gen(function* () {
        ...
        // Command Center's driver scrubs the environment it hands git, so the
        // usual `GIT_ALLOW_PROTOCOL` escape never reaches the child; pass the
        // allowance as command-line config on the one call that needs it.
        ...
        const cwd = yield* makeTmpDir();
        const { initialBranch } = yield* initRepoWithCommit(cwd);
        yield* git(cwd, [...allowFileTransport, "submodule", "add", submoduleRepo, "shared"]);
