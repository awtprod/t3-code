...
// `command-center service update`. Keep runtime imports limited to Node built-ins.
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(
        `Selected @awtprod/command-center@${version} runtime is missing or incomplete.`,
      );
    }
    if (this.#stopping) return;
...
async function main(): Promise<void> {
  const baseDir = process.env.COMMAND_CENTER_HOME?.trim() || process.env.T3CODE_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("COMMAND_CENTER_HOME is required by the Command Center service launcher.");
  }
