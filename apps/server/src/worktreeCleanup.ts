...
// Separate from the settings-configurable artifact-pruning threshold: even
// with an aggressive setting, the destructive worktree-removal step itself
// never fires before this fixed floor.
...
const WorktreeCleanupNoticeReasonSchema = Schema.Literals([
  "local-changes",
  "local-files",
  "no-upstream",
  "unpushed-commits",
...
const CleanupNoticeSchema = Schema.Struct({
  id: Schema.String,
  worktreePath: Schema.String,
  projectTitle: Schema.String,
  branch: Schema.NullOr(Schema.String),
  reason: WorktreeCleanupNoticeReasonSchema,
...
const CleanupState = Schema.Struct({
  ...
  notices: Schema.Array(CleanupNoticeSchema),
});
...
const EMPTY_STATE: CleanupState = { version: 1, entries: [], notices: [] };
...
// Marker files make same-named source directories safe across language
// ecosystems: a bare "build" directory is never pruned unless its Gradle
// sibling is also present, etc.
...
export class WorktreeCleanup extends Context.Service<
  WorktreeCleanup,
  {
    ...
    // Recovery path for a worktree this service removed: recreates it from
    // the thread's branch and re-runs the project setup script. Exposed for
    // whichever turn-start path decides a thread's worktree needs to exist;
    // not wired to one in this pass — see the notice/settings toggle for the
    // interim way to keep a worktree from being retired in the first place.
...
>()("@awtprod/command-center/worktreeCleanup") {}
...
const make = Effect.gen(function* () {
  ...
  const publishState = Effect.fn("WorktreeCleanup.publishState")(function* (state: CleanupState) {
    ...
    yield* persistState(state).pipe(
      Effect.catchCause((cause) =>
        Effect.logWarning("worktree cleanup could not persist state", { cause }),
      ),
    );
    yield* PubSub.publish(noticePubSub, state.notices);
  });

  // Every filesystem write below is preceded by this check: the candidate
  // must resolve (after following symlinks) to somewhere inside the managed
  // worktrees root, or nothing happens. This is the one guard that makes the
  // whole feature safe to run unattended.
  const managedExistingPath = Effect.fn("WorktreeCleanup.managedExistingPath")(function* (
    candidate: string,
  ) {
    // Re-resolved locally rather than captured from `make`'s enclosing
    // generator, so this function's requirement channel stays self-contained.
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
  ...
  // Local changes, untracked files, no upstream, or unpushed commits all
  // block the destructive removal step — only a notice is raised, never a
  // forced delete.
  ...
  // One-shot snapshot of which threads currently have a running terminal.
  // TerminalManager has no direct list query; subscribeMetadata delivers a
  // synchronous "snapshot" event first, so capture that and unsubscribe.
  const runningTerminalThreadIds = Effect.gen(function* () {
    const deferred = yield* Deferred.make<ReadonlySet<string>>();
  ...
  const runSweepOnce = Effect.fn("WorktreeCleanup.runSweep")(function* () {
    ...
    const [active, archived, runningTerminals, currentState, now] = yield* Effect.all([
    ...
    for (const group of groups) {
      const lastUpdatedMs = Date.parse(group.lastUpdatedAt);
      if (Number.isNaN(lastUpdatedMs)) continue;
      const inactiveMs = Math.max(0, nowMs - lastUpdatedMs);
      if (inactiveMs < cleanupAfterDays * DAY_MS || worktreeIsBusy(group, runningTerminals)) {
        continue;
      }
  ...
  const runSweep = lock.withPermits(1)(runSweepOnce());
  ...
  const prepareForTurnOnce = Effect.fn("WorktreeCleanup.prepareForTurn")(function* (input: {
    readonly threadId: string;
    readonly projectId: string;
    readonly projectCwd: string;
    readonly branch: string | null;
    ...
    const currentState = yield* Ref.get(stateRef);
    const entry = currentState.entries.find((candidate) => candidate.worktreePath === worktreePath);
    const exists = yield* fileSystem
      .exists(normalizedWorktreePath)
      .pipe(
        Effect.mapError(
          (cause) => new WorktreePreparationError({ worktreePath, operation: "inspect", cause }),
        ),
      );
    let recreated = false;
    if (!exists) {
      ...
      yield* git
        ...
        .pipe(
          Effect.mapError(
            (cause) => new WorktreePreparationError({ worktreePath, operation: "recreate", cause }),
          ),
        );
      recreated = true;
    }
    ...
    if (!entry && !currentState.notices.some((notice) => notice.worktreePath === worktreePath)) {
      return;
    }
    const nextState: CleanupState = {
      version: 1,
      entries: currentState.entries.filter((candidate) => candidate.worktreePath !== worktreePath),
  ...
  const prepareForTurn: WorktreeCleanup["Service"]["prepareForTurn"] = (input) =>
    lock
      .withPermits(1)(prepareForTurnOnce(input))
      .pipe(
        Effect.provideService(FileSystem.FileSystem, fileSystem),
        Effect.provideService(Path.Path, path),
      );
...
export const WorktreeCleanupLive = Layer.effect(WorktreeCleanup, make);
