...
projectionSnapshotLayer("ProjectionSnapshotQuery windowed thread detail", (it) => {
  ...
  const seedFanOutThread = Effect.fnUntraced(function* () {
    ...
    for (const { turn, pendingMessage, at } of turns) {
      ...
      yield* sql`
        INSERT INTO projection_thread_activities (
          activity_id, thread_id, turn_id, correlated_message_id, tone, kind, summary,
          payload_json, created_at
        )
        VALUES (${turn + "-activity"}, 'thread-w', ${turn}, ${pendingMessage}, 'tool',
          'tool.completed', 'ran tool', '{"ok":true}', ${at})
      `;
    }
