...
/**
 * Lets a connected client replace this server with another published Command Center
...
export function isPublishedCliEntry(entryPath: string): boolean {
  return normalizeEntryPath(entryPath).includes("/node_modules/@awtprod/command-center/dist/");
}

/**
 * The update path this process can offer, or null when only a manual
 * relaunch works. "desktop-managed" — the Command Center desktop app spawned this
...
>()("@awtprod/command-center/cloud/selfUpdate/ServerSelfUpdate") {}
  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === "desktop-managed") {
      return yield* failWith(
        "This server is managed by the Command Center desktop app; update the desktop app to update it.",
      );
    }
    if (capability === null) {
      return yield* failWith(
    ...
    if (!EXACT_VERSION_PATTERN.test(targetVersion)) {
      return yield* failWith(`'${targetVersion}' is not an exact Command Center version.`);
    }
    ...
    return yield* Effect.gen(function* () {
      ...
      const runtimePaths = yield* ensurePinnedRuntimeInstalled({
        ...
        Effect.mapError((error) =>
          failWith("Could not install the requested Command Center version.", error),
        ),
      );
      ...
      const preflight = yield* runner
        ...
        .pipe(
          Effect.mapError((cause) =>
            failWith(`Could not verify the installed Command Center ${targetVersion}.`, cause),
          ),
        );
      ...
      if (preflight.code !== 0 || reportedVersion !== targetVersion) {
        ...
        yield* removePinnedRuntimeInstallation({
          ...
          Effect.mapError((error) =>
            failWith(
              `Could not remove the failed Command Center ${targetVersion} installation.`,
              error,
            ),
          ),
        );
        return yield* failWith(
          preflight.code !== 0
            ? `The installed Command Center ${targetVersion} failed its version check (exit code ${String(preflight.code)}).`
            : `The installed runtime did not report the requested Command Center ${targetVersion} version.`,
        );
      }
      ...
      if (activeMethod === "boot-service") {
        ...
        yield* host
          ...
          .pipe(
            Effect.mapError((cause) =>
              failWith("Could not start the replacement Command Center process.", cause),
            ),
          );
        ...
        yield* scheduleRestart(
          Effect.try({
            try: () => host.exitProcess(),
              ...
              failWith("Could not exit the replaced Command Center process.", cause),
          }).pipe(
            Effect.catch((error) =>
