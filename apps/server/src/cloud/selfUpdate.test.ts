    ...
    ProcessRunner.ProcessRunner.of({
      run: (input) =>
        Effect.sync(() => {
          ...
          const normalizedEntryPath = (input.args[0] ?? "").replaceAll("\\", "/");
          const versionFromPath =
            input.command === NODE_PATH && input.args[1] === "--version"
              ? /\/runtime\/versions\/([^/]+)/.exec(normalizedEntryPath)?.[1]
              : undefined;
          return {
            stdout:
              options?.stdoutFor?.(input.command, input.args) ??
              (versionFromPath === undefined ? "" : `command-center v${versionFromPath}\n`),
...
it("recognizes only Command Center npm artifacts as swappable entry points", () => {
  assert.isTrue(
    SelfUpdate.isPublishedCliEntry(
      "/usr/local/lib/node_modules/@awtprod/command-center/dist/bin.mjs",
    ),
  );
  assert.isTrue(
    SelfUpdate.isPublishedCliEntry(
      "/opt/example/.npm/_npx/abc123/node_modules/@awtprod/command-center/dist/bin.mjs",
    ),
  );
  assert.isTrue(
    SelfUpdate.isPublishedCliEntry(
      "C:\\Users\\theo\\AppData\\Roaming\\npm\\node_modules\\@awtprod\\command-center\\dist\\bin.mjs",
    ),
  );
  assert.isFalse(SelfUpdate.isPublishedCliEntry("/usr/local/lib/node_modules/t3/dist/bin.mjs"));
  // Dev checkouts and the desktop bundle run apps/server/dist directly.
  assert.isFalse(SelfUpdate.isPublishedCliEntry("/opt/example/dev/t3/apps/server/dist/bin.mjs"));
  ...
  it.effect("reports boot-service for the systemd-spawned unit process", () =>
    Effect.gen(function* () {
      ...
      const entryPath = path.join(
        home,
        ".t3/runtime/versions/0.0.28/node_modules/@awtprod/command-center/dist/bin.mjs",
      );
    ...
    Effect.gen(function* () {
      ...
      const method = yield* SelfUpdate.resolveServerSelfUpdateCapability({
        ...
        provideHostRefs({
          platform: "darwin",
          env: { HOME: home },
          entryPath: `${home}/.npm/_npx/abc123/node_modules/@awtprod/command-center/dist/bin.mjs`,
        }),
      );
      assert.equal(method, "respawn");
    }),
...
it.layer(NodeServices.layer)("ServerSelfUpdate.update", (it) => {
  ...
  it.effect("rejects dist-tags and other non-exact versions", () =>
    Effect.gen(function* () {
      ...
      assert.include(error.reason, "not an exact Command Center version");
  ...
  it.effect("fails without touching anything when no update method applies", () =>
    Effect.gen(function* () {
      const context = yield* makeContext({
        entryPath: "/opt/example/dev/t3/apps/server/dist/bin.mjs",
      });
      const error = yield* context.service.update({ targetVersion: "0.0.29" }).pipe(Effect.flip);
  ...
  it.effect("surfaces a failed npm install and never schedules a restart", () =>
    Effect.gen(function* () {
      ...
      const error = yield* context.service.update({ targetVersion: "0.0.29" }).pipe(Effect.flip);
      assert.equal(error.reason, "Could not install the requested Command Center version.");
    ...
    Effect.gen(function* () {
      const context = yield* makeContext({
        stdoutFor: (command, args) =>
          command === NODE_PATH && args[1] === "--version" ? "command-center v0.0.28\n" : undefined,
      });
      const versionDir = context.path.join(context.baseDir, "runtime", "versions", "0.0.29");
  ...
  it.effect("installs, preflights, and respawns a foreground server", () =>
    Effect.gen(function* () {
      ...
      const pinnedEntry = context.path.join(
        context.baseDir,
        "runtime/versions/0.0.29/node_modules/@awtprod/command-center/dist/bin.mjs",
      );
      assert.deepEqual(
        context.commands.map((entry) => [entry.command, ...entry.args].join(" ")),
        [
          `npm install --prefix ${context.path.join(context.baseDir, "runtime/versions/0.0.29")} --no-fund --no-audit @awtprod/command-center@0.0.29`,
