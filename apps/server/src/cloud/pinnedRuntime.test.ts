...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("serializes concurrent installs of the same runtime", () =>
    Effect.gen(function* () {
      ...
      let npmRuns = 0;
      let npmArgs: ReadonlyArray<string> = [];
      ...
      assert.include(
        paths.entryPath.replaceAll("\\", "/"),
        "/node_modules/@awtprod/command-center/dist/bin.mjs",
      );
      assert.include(npmArgs, "@awtprod/command-center@0.0.29");
      assert.notInclude(npmArgs, "t3@0.0.29");
    }),
  );
