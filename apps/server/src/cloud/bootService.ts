...
/**
 * Installs Command Center as a per-user boot service. Linux-only for now: systemd
 * user unit + loginctl enable-linger. The service runs a stable or pinned
 * runtime — never an ephemeral package-runner cache whose eviction could break
 * startup.
 */
...
/**
 * Package runners (npx / pnpm dlx / bunx) use ephemeral package-manager
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Command Center background service.";
  }
}
